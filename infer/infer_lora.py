"""Async inference client for the LoRA fine-tuned model (chris241094/smolVLA-New-lora-v09).

Identical to infer.py but defaults to infer_config_lora.yaml.

Usage:
    python infer/infer_lora.py                              # uses infer_config_lora.yaml
    python infer/infer_lora.py --config path/to/other.yaml  # custom config
"""
...
def main():
    parser = argparse.ArgumentParser(description="Run async inference with the LoRA model")
    parser.add_argument(
        "--config",
        type=str,
